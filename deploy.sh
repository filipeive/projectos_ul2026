...
# Step 5: Clear Laravel caches, run composer install and run migrations
echo -e "${YELLOW}[5/6]${NC} Executando composer install, limpando caches e executando migrações..."
ssh -o StrictHostKeyChecking=no "${REMOTE_USER}@${REMOTE_HOST}" \
    "cd ${REMOTE_PATH} && composer install --no-dev --optimize-autoloader && sudo php artisan config:clear && sudo php artisan view:clear && sudo php artisan cache:clear && sudo php artisan migrate --force" 2>&1
echo -e "${GREEN}  ✓ Dependências instaladas, caches limpos e migrações executadas${NC}"
